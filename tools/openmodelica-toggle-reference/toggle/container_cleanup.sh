...
cleanup_container() {
  ...
  if ! valid_container_id "$cleanup_id"; then
    cleanup_id=
    if ! cleanup_matches=$(run_timed 5 docker ps -aq --no-trunc --filter "label=oce.toggle.run=$cleanup_label" 2>/dev/null); then
      printf 'OpenModelica Toggle cleanup query failed for run label %s\n' "$cleanup_label" >&2
      return 1
    fi
    cleanup_count=$(printf '%s\n' "$cleanup_matches" | sed '/^$/d' | wc -l | tr -d ' ')
    if [ "$cleanup_count" -gt 1 ]; then
      printf 'OpenModelica Toggle cleanup found multiple containers for run label %s; manual cleanup required\n' "$cleanup_label" >&2
      return 1
    fi
  ...
  if ! valid_container_id "$cleanup_id"; then
    printf 'OpenModelica Toggle cleanup rejected invalid container ID for run label %s\n' "$cleanup_label" >&2
    return 1
  fi
  if ! run_timed 5 docker rm -f "$cleanup_id" >/dev/null 2>&1; then
    printf 'OpenModelica Toggle cleanup failed for container %s (run label %s)\n' "$cleanup_id" "$cleanup_label" >&2
    return 1
  fi
  return 0
}
