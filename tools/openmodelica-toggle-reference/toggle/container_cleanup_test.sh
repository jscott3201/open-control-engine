...
ROOT="$SCRIPT_DIR/.container-cleanup-test-$$"
nonce=0
...
run_timed() {
  ...
  test "$1" = docker; shift
  if [ "$1" = ps ]; then
    printf 'ps:%s;' "$*" >> "$CALL_LOG"
    test "$PS_STATUS" -eq 0 || return "$PS_STATUS"
    printf '%s' "$PS_RESULT"
  else
    printf 'rm:%s;' "$*" >> "$CALL_LOG"
    return "$RM_STATUS"
  fi
}
...
test "$(cat "$CALL_LOG")" = "ps:ps -aq --no-trunc --filter label=oce.toggle.run=beta;rm:rm -f $ID_B;"
...
test "$(cat "$CALL_LOG")" = 'ps:ps -aq --no-trunc --filter label=oce.toggle.run=gamma;'
