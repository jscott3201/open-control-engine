...
case "$(uname -m)" in
  ...
  *) printf '%s\n' 'native host must be arm64/aarch64' >&2; exit 1 ;;
esac
test "$HOST_UID" -ne 0
...
cleanup() {
  if [ -n "${RUN_LABEL-}" ]; then
    cleanup_container "$RUN_LABEL" "${ACTIVE_CID_FILE-}" "${ACTIVE_CID-}" || true
  fi
  if [ "$REGENERATION_COMPLETE" -ne 1 ] && [ -n "$OUTPUT_PRIVATE" ]; then
    if ! python3 "$PUBLISH_HELPER" cleanup "$OUTPUT_PRIVATE" "$OUTPUT_DEVICE" "$OUTPUT_INODE" >/dev/null 2>&1; then
      printf 'OpenModelica Toggle private-output cleanup failed for %s\n' "$OUTPUT_PRIVATE" >&2
    fi
  fi
}
trap cleanup EXIT
...
OUTPUT_RECORD=$(python3 "$PUBLISH_HELPER" claim "$OUTPUT_REQUESTED")
...
OCI_DIR="$REPO_ROOT/crates/oce-conformance/tests/fixtures/open_modelica/logical_toggle"
...
check_hash 49d33a88242163def412dff83e3f49e23b187fe56859a01649edbf5fb2bab60c "$STAGING/sources/buildings/Buildings/Controls/OBC/CDL/Logical/Toggle.mo"
check_hash 2ab7e15a0026d5a6c0089d7af88329869fc3e397398a7b86ad5ea49fd1d9b271 "$STAGING/sources/buildings/Buildings/Controls/OBC/CDL/Logical/Latch.mo"
...
run_model() {
  ...
  mkdir "$directory"
  ACTIVE_CID_FILE="$directory/container.cid"
  ACTIVE_CID=
  log="$directory/run.log"
  {
    ...
    printf 'toggle_source_sha256=49d33a88242163def412dff83e3f49e23b187fe56859a01649edbf5fb2bab60c\n'
    printf 'latch_source_sha256=2ab7e15a0026d5a6c0089d7af88329869fc3e397398a7b86ad5ea49fd1d9b271\n'
  ...
  deadline_start 120
  deadline_call docker run -d --cidfile "$ACTIVE_CID_FILE" --label "oce.toggle.run=$RUN_LABEL" --name "$container" --pull=never --platform linux/arm64 \
  ...
  ACTIVE_CID=$(cat "$ACTIVE_CID_FILE")
  ...
  while :; do
    ...
    if [ "$poll_status" -eq 0 ]; then
      ...
      deadline_call docker exec "$ACTIVE_CID" cat /out/TogglePilot_res.csv > "$directory/TogglePilot_res.csv"
      # Cleanup has its own bounded allowance and is not execution time.
      run_timed 5 docker kill "$ACTIVE_CID" >/dev/null
      run_timed 5 docker rm "$ACTIVE_CID" >/dev/null
      rm "$ACTIVE_CID_FILE"
      ACTIVE_CID_FILE=
      ACTIVE_CID=
      return 0
    ...
    if ! deadline_refresh; then run_timed 5 docker kill "$ACTIVE_CID" >/dev/null 2>&1 || true; return 1; fi
    if [ "$inspect_status" -ne 0 ] || [ "$inspect_state" != true ]; then
      deadline_call docker logs "$ACTIVE_CID" >> "$log" 2>&1 || true
      return 1
    fi
    if ! deadline_refresh || [ "$DEADLINE_REMAINING" -le 1 ]; then return 1; fi
    sleep 1
...
run_model fresh-run-a Toggle "$RUN_A_CONTAINER" "$OUTPUT/run-a"
run_model fresh-run-b Toggle "$RUN_B_CONTAINER" "$OUTPUT/run-b"
run_model fresh-semantic-control Latch "$CONTROL_CONTAINER" "$OUTPUT/semantic-control"
cmp "$OUTPUT/run-a/TogglePilot_res.csv" "$OUTPUT/run-b/TogglePilot_res.csv"
if cmp -s "$OUTPUT/run-a/TogglePilot_res.csv" "$OUTPUT/semantic-control/TogglePilot_res.csv"; then
  printf '%s\n' 'semantic control did not mutate raw output' >&2
  exit 1
fi
cargo run --manifest-path "$REPO_ROOT/tools/openmodelica-toggle-reference/Cargo.toml" --offline --locked -- \
  canonicalize "$OUTPUT/run-a/TogglePilot_res.csv" "$OUTPUT/toggle.canonical.csv" openmodelica_logical_toggle
cargo run --manifest-path "$REPO_ROOT/tools/openmodelica-toggle-reference/Cargo.toml" --offline --locked -- \
  canonicalize "$OUTPUT/semantic-control/TogglePilot_res.csv" "$OUTPUT/latch.canonical.csv" openmodelica_logical_latch

mv "$OUTPUT/run-a/TogglePilot_res.csv" "$OUTPUT/toggle-run-a.raw.csv"
mv "$OUTPUT/run-b/TogglePilot_res.csv" "$OUTPUT/toggle-run-b.raw.csv"
mv "$OUTPUT/semantic-control/TogglePilot_res.csv" "$OUTPUT/latch.raw.csv"
mv "$OUTPUT/run-a/run.log" "$OUTPUT/run-a.log"
mv "$OUTPUT/run-b/run.log" "$OUTPUT/run-b.log"
mv "$OUTPUT/semantic-control/run.log" "$OUTPUT/latch.log"
rmdir "$OUTPUT/run-a" "$OUTPUT/run-b" "$OUTPUT/semantic-control"
cp "$OCI_DIR/image-index.json" "$OUTPUT/image-index.json"
cp "$OCI_DIR/image-manifest.json" "$OUTPUT/image-manifest.json"

MUTATION=$(mktemp -d "$STAGING/mutation.XXXXXX")
mkdir -p "$MUTATION/repo/tools/openmodelica-toggle-reference/src" \
  "$MUTATION/repo/crates/oce-cxf/tests/open_modelica_toggle_reference"
cp "$REPO_ROOT/tools/openmodelica-toggle-reference/Cargo.toml" \
  "$REPO_ROOT/tools/openmodelica-toggle-reference/Cargo.lock" \
  "$MUTATION/repo/tools/openmodelica-toggle-reference/"
cp "$REPO_ROOT/tools/openmodelica-toggle-reference/src/main.rs" \
  "$MUTATION/repo/tools/openmodelica-toggle-reference/src/main.rs"
CANONICALIZER="$REPO_ROOT/crates/oce-cxf/tests/open_modelica_toggle_reference/canonicalizer.rs"
cp "$CANONICALIZER" "$MUTATION/repo/crates/oce-cxf/tests/open_modelica_toggle_reference/canonicalizer.rs"
perl -0pi -e 's/            \*rows\.last_mut\(\)\.expect\("equal-time group exists"\) = row;\n//' \
  "$MUTATION/repo/crates/oce-cxf/tests/open_modelica_toggle_reference/canonicalizer.rs"
cargo run --manifest-path "$MUTATION/repo/tools/openmodelica-toggle-reference/Cargo.toml" \
  --offline --locked -- canonicalize-inspect "$OUTPUT/toggle-run-a.raw.csv" \
  "$MUTATION/keep-first.canonical.csv" openmodelica_logical_toggle_keep_first \
  "$MUTATION/keep-first.metadata"
MISMATCHES=$(cargo run --manifest-path "$MUTATION/repo/tools/openmodelica-toggle-reference/Cargo.toml" \
  --offline --locked --quiet -- schedule-mismatches "$MUTATION/keep-first.canonical.csv")
test "$MISMATCHES" = 'schedule_mismatch_rows=1,3,5,7,9,11,12,14,16,17,19'
...
mutated_input_sha256=$(shasum -a 256 "$OUTPUT/toggle-run-a.raw.csv" | cut -d' ' -f1)
mutated_raw_rows=34
...
mutated_schedule_first_mismatch_time_bits=403e000000001dff
...
python3 "$PUBLISH_HELPER" cleanup "$STAGING" "$STAGING_DEVICE" "$STAGING_INODE"
STAGING=
python3 "$SCRIPT_DIR/generate_manifest.py" "$OUTPUT" "$REPO_ROOT"
python3 "$SCRIPT_DIR/verify_evidence.py" "$OUTPUT" "$REPO_ROOT"
printf 'toggle_raw_sha256='; shasum -a 256 "$OUTPUT/toggle-run-a.raw.csv" | cut -d' ' -f1
printf 'latch_raw_sha256='; shasum -a 256 "$OUTPUT/latch.raw.csv" | cut -d' ' -f1
# Atomic publication is the final operation; do not expose a signal window before state is recorded.
trap '' HUP INT TERM
python3 "$PUBLISH_HELPER" publish "$OUTPUT" "$OUTPUT_DEVICE" "$OUTPUT_INODE" \
  "$OUTPUT_PARENT_DEVICE" "$OUTPUT_PARENT_INODE" "$OUTPUT_DESTINATION"
OUTPUT_PRIVATE=
