...
def inspect_directory(path, name):
    ...
    try:
        ...
        relative = pathlib.Path(*path.parts[1:]); current = pathlib.Path(path.anchor)
    else:
        current = trusted
...
def claim(destination):
    ...
    path = pathlib.Path(tempfile.mkdtemp(prefix=".oce-reliefs-evidence.", dir=parent))
    path.chmod(0o700); metadata = path.lstat()
...
def main(arguments):
    ...
    if command == "claim" and len(values) == 1: claim(values[0])
    elif command == "claim-child" and len(values) == 2: claim_child(*values)
    elif command == "cleanup" and len(values) == 3: cleanup(*values)
    elif command == "publish" and len(values) == 6: publish(*values)
