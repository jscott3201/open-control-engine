...
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd); HELPER="$SCRIPT_DIR/output_publish.py"
SYSTEM_TEMP=$(env -u TMPDIR -u TMP -u TEMP python3 -c 'import pathlib,tempfile; print(pathlib.Path(tempfile.gettempdir()).resolve())')
PRIVATE_PARENT="$SYSTEM_TEMP/oce-reliefs-publish-tests-$(id -u)"
...
ROOT=$(printf '%s' "$ROOT_RECORD" | cut -f1); ROOT_DEVICE=$(printf '%s' "$ROOT_RECORD" | cut -f2); ROOT_INODE=$(printf '%s' "$ROOT_RECORD" | cut -f3)
cleanup() { env -u TMPDIR -u TMP -u TEMP python3 "$HELPER" cleanup "$ROOT" "$ROOT_DEVICE" "$ROOT_INODE"; rmdir "$PRIVATE_PARENT" 2>/dev/null || true; }
trap cleanup EXIT HUP INT TERM
claim() {
  record=$(python3 "$HELPER" claim "$1"); path=$(printf '%s' "$record" | cut -f1); device=$(printf '%s' "$record" | cut -f2); inode=$(printf '%s' "$record" | cut -f3)
  parent_device=$(printf '%s' "$record" | cut -f4); parent_inode=$(printf '%s' "$record" | cut -f5); destination=$(printf '%s' "$record" | cut -f6)
}
claim "$ROOT/published"; printf '%s\n' evidence > "$path/marker"
...
claim "$ROOT/replaced-output"; rmdir "$path"; mkdir "$ROOT/replacement"; printf '%s\n' protected > "$ROOT/replacement/marker"; ln -s "$ROOT/replacement" "$path"
if python3 "$HELPER" cleanup "$path" "$device" "$inode" >/dev/null 2>&1; then exit 1; fi
test "$(cat "$ROOT/replacement/marker")" = protected; rm "$path"
claim "$ROOT/blocked-output"; ln -s "$ROOT/replacement" "$destination"
if python3 "$HELPER" publish "$path" "$device" "$inode" "$parent_device" "$parent_inode" "$destination" >/dev/null 2>&1; then exit 1; fi
test -d "$path"; rm "$destination"; python3 "$HELPER" cleanup "$path" "$device" "$inode"
