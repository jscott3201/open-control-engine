...
*source semantics* — 412 Tier-A provenance records (410 signal goldens plus two fold-time
provenance-only records), behind a CI-enforced code-dependency firewall.
...
Correctness is bounded by a separate layer generated by `tools/golden-gen` — a crate held off the
workspace and **forbidden from depending on `oce-blocks`**. Its references are re-derived from
CDL / Buildings source semantics rather than imported from the implementation under test. CI
enforces that code-dependency firewall, and every golden records `depends_on_oce_blocks: false`.
For classes whose oracle shares a pinned math kernel or restates the same documented recurrence,
a Tier-A pass checks plumbing and transcription of the shared formula, not the formula
independently; a mechanical shared-kernel detector is filed as follow-up. The layer contains
**412 Tier-A provenance records: 280 CDL records (278 signal goldens plus two fold-time
provenance-only records), plus 132 G36 sequence signal goldens spanning all 46 fixtures**. The
410 signal goldens are compared **bit-exactly** — the 278 CDL signals through the per-block
harness, the 132 G36 signals through per-sequence oracle suites in
...
Per-block Tier-A goldens cover **128 of 133 CDL classes**. `CDL.Logical.Nor`,
`CDL.Logical.Pre`, `CDL.Logical.Sources.Constant`, and `CDL.Reals.MovingAverage` carry indirect
G36-sequence evidence only; `CDL.Utilities.Assert` has no output port, and its
diagnostics-channel golden is filed.
