...
plus 132 G36 sequence outputs spanning all 46 fixtures**, compared through the funnel suites in
`crates/oce-conformance/tests/`. Most are closed-form derivations; some, such as
`TimeSuppression`, are explicit per-tick recurrences.
...
- **Deterministic on the tick** — a frozen, topologically-sorted schedule evaluated over flat
  ...
  both real:
  - **The evaluator is not unconditionally allocation-free.** `Reals.Log` and `Reals.Log10`
    build a diagnostic string with `format!` when handed a non-positive input, and that allocates
    before the sink is even called. Allocation-freedom holds on the arithmetic path, not on every
    warning path.
...
| `oce-expr` | The CDL §7.7.2 binding-expression parser/evaluator (closed-world, pure). Total over the value domain; **not** over arbitrary nesting depth — neither the parser nor the evaluator carries a depth guard, so sufficiently deep input can exhaust the thread stack. Hardening is tracked. |
