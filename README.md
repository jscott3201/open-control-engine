...
references are re-derived from the CDL spec math rather than from the implementation under test.
CI enforces that firewall, and every golden it emits records `depends_on_oce_blocks: false`. It
covers **407 block- and type-level Tier-A goldens**, and at sequence level **9 of the 46 carry
Tier-A oracles over 32 output signals**. So the block library is independently checked far more
broadly than the whole-sequence layer is. **What is not wired is Tier 3 — cross-implementation differential against an external
