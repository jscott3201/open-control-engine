//! `CDL.Routing` Boolean-family algebraic blocks.
//!
//! These classes lower CDL Boolean vector and matrix connectors to flattened scalar ports at BUILD
...
use crate::{
    ...
    ResolvedBlockSignature, read_bool, read_int,
};

const EXTRACTOR_WARNING: &str = "The extract index is out of the range.";

fn bool_kinds(width: usize) -> Vec<PortKind> {
    PortShape::new(PortKind::Boolean, width).to_kinds()
}
...
/// `CDL.Routing.BooleanExtractSignal` extracts a parameter-selected Boolean vector.
///
...
/// deterministically for direct invalid construction by emitting `false` when no input exists or by
...
impl Block for BooleanExtractSignal {
    ...
    fn step_algebraic(&self, _ctx: &Ctx<'_>, inputs: &[Value], emit: &mut dyn FnMut(usize, Value)) {
        for out_idx in 0..self.outputs.len() {
            let y = self
                ...
                .is_some_and(|in_idx| read_bool(inputs, in_idx));
            emit(out_idx, Value::Boolean(y));
        }
    }
}

/// `CDL.Routing.BooleanExtractor` selects one Boolean vector element by runtime integer index.
///
/// The block is stateless `[A]`. Source behavior warns, clamps `index` into `1..=nin`, and emits
/// the selected input. Because the selected Boolean input is runtime-dependent, feedthrough is
/// conservative from `index` and every resolved `u[i]` to `y`. Direct construction with `nin=0`
/// emits `false` and warns instead of panicking; load-time validation rejects that source-invalid
/// shape.
#[derive(Clone, Debug, Default)]
...
impl Block for BooleanExtractor {
    ...
    fn step_algebraic(&self, ctx: &Ctx<'_>, inputs: &[Value], emit: &mut dyn FnMut(usize, Value)) {
        ...
        emit(0, Value::Boolean(read_bool(inputs, clamped)));
    }
}

/// `CDL.Routing.BooleanScalarReplicator` fills a Boolean vector with one scalar input.
///
/// The block is stateless `[A]`, feeds the scalar input through to every resolved output, and emits
/// no outputs when `nout=0`. No method panics.
#[derive(Clone, Debug, Default)]
pub struct BooleanScalarReplicator {
    outputs: Vec<PortKind>,
}

impl BooleanScalarReplicator {
...
impl Block for BooleanScalarReplicator {
    fn signature(&self) -> &'static BlockSignature {
        static SIG: BlockSignature = BlockSignature {
    ...
    fn step_algebraic(&self, _ctx: &Ctx<'_>, inputs: &[Value], emit: &mut dyn FnMut(usize, Value)) {
        ...
        for out_idx in 0..self.outputs.len() {
            emit(out_idx, Value::Boolean(u));
        }
    }
}

/// `CDL.Routing.BooleanVectorFilter` emits Boolean vector elements whose structural mask is true.
///
...
/// corresponding output. Direct invalid construction emits `false` for outputs without a selected
/// source input rather than panicking.
#[derive(Clone, Debug, Default)]
pub struct BooleanVectorFilter {
    inputs: Vec<PortKind>,
    outputs: Vec<PortKind>,
    selected: Vec<usize>,
}

impl BooleanVectorFilter {
...
impl Block for BooleanVectorFilter {
    fn signature(&self) -> &'static BlockSignature {
        static SIG: BlockSignature = BlockSignature {
    ...
    fn step_algebraic(&self, _ctx: &Ctx<'_>, inputs: &[Value], emit: &mut dyn FnMut(usize, Value)) {
        for out_idx in 0..self.outputs.len() {
            let y = self
                ...
                .is_some_and(|idx| read_bool(inputs, *idx));
            emit(out_idx, Value::Boolean(y));
        }
    }
}

/// `CDL.Routing.BooleanVectorReplicator` fills each output matrix row with the input vector.
///
...
#[derive(Clone, Debug, Default)]
pub struct BooleanVectorReplicator {
    inputs: Vec<PortKind>,
    outputs: Vec<PortKind>,
    nin: usize,
}

impl BooleanVectorReplicator {
    pub(crate) fn new(nin: usize, nout: usize) -> Self {
        let output_width = bounded_product_width(nin, nout);
        Self {
            inputs: bool_kinds(nin),
            outputs: bool_kinds(output_width),
            nin,
        }
    }
}

impl Block for BooleanVectorReplicator {
    fn signature(&self) -> &'static BlockSignature {
        static SIG: BlockSignature = BlockSignature {
    ...
    fn step_algebraic(&self, _ctx: &Ctx<'_>, inputs: &[Value], emit: &mut dyn FnMut(usize, Value)) {
        ...
        for out_idx in 0..self.outputs.len() {
            emit(
                out_idx,
                Value::Boolean(read_bool(inputs, out_idx % self.nin)),
            );
        }
    }
}
