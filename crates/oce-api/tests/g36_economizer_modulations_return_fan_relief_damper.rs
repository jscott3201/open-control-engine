...
const EXPECTED_RELIEF_DAMPER_COMMAND: [f64; 7] = [0.0, 0.0, 0.25, 0.5, 0.75, 1.0, 1.0];
...
fn load_economizer_modulations_return_fan_relief_damper() -> Engine {
    let mut engine = Engine::in_memory();
    let report = engine
        .load_cxf(ECONOMIZER_MODULATIONS_RETURN_FAN_RELIEF_DAMPER.as_bytes())
        .expect("source-verified G36 Economizers.Subsequences.Modulations.ReturnFan relief-damper variant fixture loads");
...
#[test]
fn multizone_vav_economizer_modulations_return_fan_relief_damper_loads_simulates_and_is_deterministic()
 {
    let engine = load_economizer_modulations_return_fan_relief_damper();
    let points = engine
        .io()
        .iter()
    ...
    assert_real_bits(
        &real_column(&metrics, RELIEF_DAMPER_COMMAND),
        &EXPECTED_RELIEF_DAMPER_COMMAND,
    ...
    let (second_schedule, second_metrics) =
        simulate(load_economizer_modulations_return_fan_relief_damper());
