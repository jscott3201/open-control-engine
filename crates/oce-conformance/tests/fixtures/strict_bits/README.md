...
17-source map and cannot qualify the strengthened checker. The active current-qualification test
requires all 35 source digests, current oracle/input/CXF integrity and a new native matrix.
Later HEAD equality is deliberately not required; no source-map exception or rewritten provenance
is used. `receipts.json.current` is currently null, so that test explicitly refuses with
`PENDING_NATIVE_EVIDENCE`. This is a native-evidence staging head, not a green delivery.

The ignored `admit_downloaded_native_matrix_without_rewriting_provenance` test is the explicit
local admission path. After reviewing the new native receipt into `receipts.json.current`, with
`OCE_STRICT_MATRIX_DIR` naming the downloaded artifact directory, it checks that exact receipt,
