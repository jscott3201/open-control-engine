//! G36 ReliefFan Tier-A independent-oracle check through the B3 facade driver.
...
const OUTPUTS: &[PointSpec] = &[
    ...
    PointSpec::boolean("relief_damper_status", RELIEF_DAMPER_STATUS),
    PointSpec::boolean("relief_fan_status", RELIEF_FAN_STATUS),
    PointSpec::real("relief_fan_speed", RELIEF_FAN_SPEED),
];
...
#[test]
fn g36_relief_fan_tier_a_oracle_matches_engine_output() {
    let reference = CombiTimeTable::read(&reference_path())
        .unwrap_or_else(|err| panic!("ReliefFan reference read failed: {err}"));
    assert_eq!(reference.name, "G36_multizone_vav_relief_fan_reference");
    assert_eq!(reference.n_rows, 30);
    assert_eq!(
    ...
    let run = drive_trace_with_options(
        ...
        &DriverOptions {
            cadence: DriveCadence::EventAligned {
                instants: (0..=29).map(|tick| f64::from(tick) * 60.0).collect(),
            },
            input_replay: DriverInputReplay::ReferenceTable,
            comparison: ComparisonMode::Exact,
        },
    )
    .unwrap_or_else(|err| panic!("ReliefFan exact driver run failed: {err}"));
    ...
    for output in OUTPUTS {
        ...
        match &comparison.result {
            ComparisonResult::Exact(result) => {
                assert!(
                    result.passed,
                    "ReliefFan exact comparison failed for {}: {:?}",
                    output.reference_name, result.first_mismatch
                );
                assert_eq!(result.compared_points, 30);
                assert_eq!(result.first_mismatch, None);
            }
            other => panic!("ReliefFan used non-exact comparison: {other:?}"),
        }
    }
}
