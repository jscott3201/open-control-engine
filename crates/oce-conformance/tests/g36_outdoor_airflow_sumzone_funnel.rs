//! G36 OutdoorAirFlow ASHRAE 62.1 SumZone Tier-A independent-oracle check through the B3 facade driver.
...
const SEQUENCE: &str = "multizone_vav_outdoor_airflow_sumzone";
...
const INPUTS: &[PointSpec] = &[
    ...
    PointSpec::real("population_flow_1", POP_1),
    PointSpec::real("population_flow_2", POP_2),
    PointSpec::real("population_flow_3", POP_3),
    PointSpec::real("area_flow_1", AREA_1),
    PointSpec::real("area_flow_2", AREA_2),
    PointSpec::real("area_flow_3", AREA_3),
    PointSpec::real("primary_flow_1", PRI_1),
    PointSpec::real("primary_flow_2", PRI_2),
    PointSpec::real("primary_flow_3", PRI_3),
    PointSpec::real("minimum_outdoor_air_1", MIN_OA_1),
    PointSpec::real("minimum_outdoor_air_2", MIN_OA_2),
    PointSpec::real("minimum_outdoor_air_3", MIN_OA_3),
];
const OUTPUTS: &[PointSpec] = &[
    ...
    PointSpec::real("summed_adjusted_area_breathing_zone_flow", SUMMED_AREA_FLOW),
    PointSpec::real("summed_zone_primary_airflow", SUMMED_PRIMARY_FLOW),
    PointSpec::real(
...
#[test]
fn g36_outdoor_airflow_sumzone_tier_a_oracle_matches_engine_output() {
    let reference = CombiTimeTable::read(&reference_path()).unwrap_or_else(|err| {
        panic!("OutdoorAirFlow ASHRAE62_1 SumZone reference read failed: {err}")
    });
    assert_eq!(
        reference.name,
        "G36_multizone_vav_outdoor_airflow_sumzone_reference"
    );
    ...
    let run = drive_trace_with_options(
        OUTDOOR_AIRFLOW_SUMZONE.as_bytes(),
        &config(),
        &reference,
        &DriverOptions {
    ...
    .unwrap_or_else(|err| {
        panic!("OutdoorAirFlow ASHRAE62_1 SumZone exact driver run failed: {err}")
    });
    ...
    for output in OUTPUTS {
        ...
        match &comparison.result {
            ComparisonResult::Exact(result) => {
                assert!(
                    result.passed,
                    "OutdoorAirFlow ASHRAE62_1 SumZone exact comparison failed for {}: {:?}",
            ...
            other => {
                panic!("OutdoorAirFlow ASHRAE62_1 SumZone used non-exact comparison: {other:?}")
            }
        }
    }
}
...
#[test]
fn funnel_band_routes_outdoor_airflow_sumzone_real_outputs() {
    ...
    funnel_band_policy::route_real_outputs_through_funnel_band(
        &funnel_band_policy::FunnelRouting {
            sequence: SEQUENCE,
            cxf: OUTDOOR_AIRFLOW_SUMZONE.as_bytes(),
            inputs: &inputs,
            outputs: &outputs,
            instants: &instants,
            sample_step: 1.0,
