...
const SEQUENCE: &str = "multizone_vav_economizer_modulations_return_fan";
const ROWS: usize = 7;
...
#[test]
fn g36_economizer_modulations_return_fan_tier_a_oracle_matches_engine_output() {
    let reference = CombiTimeTable::read(&reference_path()).unwrap_or_else(|err| {
        panic!("Economizer Modulations.ReturnFan reference read failed: {err}")
    });
    assert_eq!(
        reference.name,
        "G36_multizone_vav_economizer_modulations_return_fan_reference"
    );
    ...
    let run = drive_trace_with_options(
        ECONOMIZER_MODULATIONS_RETURN_FAN.as_bytes(),
        &config(),
        &reference,
        &DriverOptions {
    ...
    .unwrap_or_else(|err| {
        panic!("Economizer Modulations.ReturnFan exact driver run failed: {err}")
    });
    ...
    for output in OUTPUTS {
        ...
        match &comparison.result {
            ComparisonResult::Exact(result) => {
                assert!(
                    result.passed,
                    "Economizer Modulations.ReturnFan exact comparison failed for {}: {:?}",
            ...
            other => {
                panic!("Economizer Modulations.ReturnFan used non-exact comparison: {other:?}")
            }
        }
    }
}
...
#[test]
fn funnel_band_routes_economizer_modulations_return_fan_real_outputs() {
    ...
    funnel_band_policy::route_real_outputs_through_funnel_band(
        &funnel_band_policy::FunnelRouting {
            sequence: SEQUENCE,
            cxf: ECONOMIZER_MODULATIONS_RETURN_FAN.as_bytes(),
            inputs: &inputs,
            outputs: &outputs,
            instants: &instants,
