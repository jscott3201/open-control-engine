//! G36 Economizers.Subsequences.Limits.Common Tier-A independent-oracle check through the B3 facade driver.
...
const SEQUENCE: &str = "multizone_vav_economizer_limits_common";
const ROWS: usize = 8;
...
const OUTPUTS: &[PointSpec] = &[
    ...
    PointSpec::real("outdoor_damper_max_limit", OUTDOOR_DAMPER_MAX_LIMIT),
    PointSpec::real("return_damper_min_limit", RETURN_DAMPER_MIN_LIMIT),
    PointSpec::real("return_damper_max_limit", RETURN_DAMPER_MAX_LIMIT),
    PointSpec::real(
...
#[test]
fn g36_economizer_limits_common_tier_a_oracle_matches_engine_output() {
    let reference = CombiTimeTable::read(&reference_path())
        .unwrap_or_else(|err| panic!("Economizer Limits.Common reference read failed: {err}"));
    assert_eq!(
        reference.name,
        "G36_multizone_vav_economizer_limits_common_reference"
    );
    ...
    let run = drive_trace_with_options(
        ECONOMIZER_LIMITS_COMMON.as_bytes(),
        &config(),
        &reference,
        &DriverOptions {
    ...
    .unwrap_or_else(|err| panic!("Economizer Limits.Common exact driver run failed: {err}"));
    ...
    for output in OUTPUTS {
        ...
        match &comparison.result {
            ComparisonResult::Exact(result) => {
                assert!(
                    result.passed,
                    "Economizer Limits.Common exact comparison failed for {}: {:?}",
