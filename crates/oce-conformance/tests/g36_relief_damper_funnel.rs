//! G36 ReliefDamper Tier-A independent-oracle check through the B3 facade driver.
...
const SEQUENCE: &str = "multizone_vav_relief_damper";
...
const OUTPUTS: &[PointSpec] = &[PointSpec::real(
    "relief_damper_command",
    RELIEF_DAMPER_COMMAND,
)];

#[test]
fn g36_relief_damper_tier_a_oracle_matches_engine_output() {
    let reference = CombiTimeTable::read(&reference_path())
        .unwrap_or_else(|err| panic!("ReliefDamper reference read failed: {err}"));
    assert_eq!(reference.name, "G36_multizone_vav_relief_damper_reference");
    ...
    .unwrap_or_else(|err| panic!("ReliefDamper exact driver run failed: {err}"));

    assert_eq!(run.comparisons.len(), OUTPUTS.len());
    let comparison = &run.comparisons[0];
    assert_eq!(comparison.output, RELIEF_DAMPER_COMMAND);
    assert_eq!(comparison.reference_column, "relief_damper_command");
    assert!(!comparison.masked);
    match &comparison.result {
        ComparisonResult::Exact(result) => {
            assert!(
                result.passed,
                "ReliefDamper exact comparison failed: {:?}",
...
fn assert_signal_provenance(reference: &CombiTimeTable) {
    let prov = read_json(&signal_provenance_path("relief_damper_command"));
    assert_eq!(prov["class_path"], "G36");
    assert_eq!(prov["scenario"], SEQUENCE);
    assert_eq!(prov["signal"], "relief_damper_command");
