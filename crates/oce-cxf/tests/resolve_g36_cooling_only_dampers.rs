//! Source-verified G36 CoolingOnly Dampers composite import tests.
//!
//! The fixture grounds the Validation parameters and the two upstream enum references before
...
const G36_COOLING_ONLY_DAMPERS: &str = include_str!("fixtures/g36/cooling_only_dampers.jsonld");
const G36_COOLING_ONLY_DAMPERS_GOLDEN_REL: &str =
    "tests/fixtures/golden/g36_cooling_only_dampers.modelgraph.txt";
const G36_COOLING_ONLY_DAMPERS_MODEL: &str = "http://example.org#g36.source.cooling_only_dampers";
const G36_COOLING_ONLY_DAMPERS_CLASS: &str = "http://example.org#Buildings.Controls.OBC.ASHRAE.G36.TerminalUnits.CoolingOnly.Subsequences.Dampers";

fn import_ok(src: &str) -> ModelGraph {
    let (graph, report) = import_cxf(src.as_bytes(), &ResolveOptions::default())
        .expect("G36 CoolingOnly Dampers fixture should import");
    assert!(
        report.is_empty(),
...
#[test]
fn source_verified_cooling_only_dampers_preserves_topology_and_grounded_bindings() {
    let parsed: JsonValue =
        serde_json::from_str(G36_COOLING_ONLY_DAMPERS).expect("G36 fixture JSON");
    let top = parsed["@graph"]
        .as_array()
        .expect("@graph array")
        .iter()
        .find(|node| node["@id"] == json!(G36_COOLING_ONLY_DAMPERS_MODEL))
        .expect("top G36 CoolingOnly Dampers composite node");
    assert_eq!(top["@type"], json!(G36_COOLING_ONLY_DAMPERS_CLASS));
    assert_eq!(
    ...
    assert_eq!(top["S231:hasInput"].as_array().expect("inputs").len(), 10);
    assert_eq!(top["S231:hasOutput"].as_array().expect("outputs").len(), 2);

    let graph = import_ok(G36_COOLING_ONLY_DAMPERS);
    assert_eq!(graph.blocks.len(), 35);
    let instances = graph
        .blocks
        .iter()
    ...
    assert!(
        block_param(&graph, ".conPID", "controllerType").bit_eq(&Value::Enum {
            class: EnumClassId::SIMPLE_CONTROLLER,
            ordinal: 2,
        })
    );
    assert!(block_param(&graph, ".conInt", "k").bit_eq(&Value::Integer(3)));
    ...
    for (suffix, expected_true) in [
        ...
        assert!(
            block_param(&graph, suffix, "realTrue").bit_eq(&Value::Real(expected_true)),
            "{suffix}.realTrue"
        );
        assert!(
            block_param(&graph, suffix, "realFalse").bit_eq(&Value::Real(0.0)),
            "{suffix}.realFalse"
        );
    }
}
...
fn cooling_only_dampers_modelgraph_is_stable() {
    ...
    assert_eq!(
        actual, expected,
        "source-verified G36 CoolingOnly Dampers ModelGraph diverged from golden"
    );
}
