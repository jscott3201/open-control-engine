...
const G36_OUTDOOR_AIRFLOW_AHU: &str =
    include_str!("fixtures/g36/multizone_vav_outdoor_airflow_ahu.jsonld");
const G36_OUTDOOR_AIRFLOW_AHU_GOLDEN_REL: &str =
    "tests/fixtures/golden/g36_multizone_vav_outdoor_airflow_ahu.modelgraph.txt";
const G36_OUTDOOR_AIRFLOW_AHU_MODEL: &str =
    "http://example.org#g36.source.multizone_vav_outdoor_airflow_ahu";
const G36_OUTDOOR_AIRFLOW_AHU_CLASS: &str = "http://example.org#Buildings.Controls.OBC.ASHRAE.G36.AHUs.MultiZone.VAV.SetPoints.OutdoorAirFlow.ASHRAE62_1.AHU";

fn import_ok(src: &str) -> ModelGraph {
    let (graph, report) = import_cxf(src.as_bytes(), &ResolveOptions::default())
        .expect("G36 OutdoorAirFlow AHU fixture should import");
    assert!(
        report.is_empty(),
...
#[test]
fn source_verified_g36_multizone_vav_outdoor_airflow_ahu_imports_single_damper_variant() {
    let parsed: JsonValue =
        serde_json::from_str(G36_OUTDOOR_AIRFLOW_AHU).expect("G36 fixture JSON");
    let top = parsed["@graph"]
        .as_array()
        .expect("@graph array")
        .iter()
        .find(|node| node["@id"] == json!(G36_OUTDOOR_AIRFLOW_AHU_MODEL))
        .expect("top G36 OutdoorAirFlow AHU composite node");
    assert_eq!(top["@type"], json!(G36_OUTDOOR_AIRFLOW_AHU_CLASS));
    assert_eq!(
        top["S231:containsBlock"]
            ...
            .len(),
        15,
        "ASHRAE62.1 AHU source transcription should preserve 15 child components"
    );
    assert_eq!(
        top["S231:hasInput"].as_array().expect("inputs").len(),
        5,
        "SingleDamper variant should keep the measured outdoor-air input"
    );
    assert_eq!(
        top["S231:hasOutput"].as_array().expect("outputs").len(),
        4,
        "SingleDamper variant should keep the measured outdoor-air normalized output"
    );

    let graph = import_ok(G36_OUTDOOR_AIRFLOW_AHU);
    assert_eq!(
        graph.blocks.len(),
        15,
        "scalar AHU-level outdoor-air sequence should import all active children"
    );
    let instances = graph
        .blocks
        .iter()
    ...
    assert!(
        block_param(&graph, ".uncDesOutAir", "k").bit_eq(&Value::Real(6.0)),
        "uncorrected design outdoor airflow must ground from parent parameter"
    );
    assert!(
        block_param(&graph, ".desOutAir", "k").bit_eq(&Value::Real(8.0)),
        "design total outdoor airflow must ground from parent parameter"
    );
    assert!(
        block_param(&graph, ".addPar", "p").bit_eq(&Value::Real(1.0)),
        "system ventilation efficiency offset must be one"
    );
    assert!(
        block_param(&graph, ".gaiDivZer", "k").bit_eq(&Value::Real(1E-3)),
        "zone-primary airflow guard gain must be 0.1 percent"
    );
    assert!(
        block_param(&graph, ".neaZer", "k").bit_eq(&Value::Real(1E-4)),
        "system-ventilation-efficiency guard must be near zero"
    );
}
...
fn golden_g36_multizone_vav_outdoor_airflow_ahu_modelgraph() {
    ...
    assert_eq!(
        actual, expected,
        "source-verified G36 OutdoorAirFlow AHU ModelGraph diverged from golden"
    );
}
