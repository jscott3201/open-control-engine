...
| `rejected/declaration_cycle.jsonld` | 5 | A reference cycle between two of the root's own parameters (`a = "b + 1.0"`, `b = "a + 1.0"`). | `composite/declaration-cycle` (`malformed-document`); ONE diagnostic per distinct cycle, subject is the participant earliest in chained declaration order (`#M.a`), message names every participant in chained order closing on the first. |
