...
const G36_REHEAT_OVERRIDES: &str = include_str!("fixtures/g36/reheat_overrides.jsonld");
const G36_REHEAT_OVERRIDES_GOLDEN_REL: &str =
    "tests/fixtures/golden/g36_reheat_overrides.modelgraph.txt";
const G36_REHEAT_OVERRIDES_MODEL: &str = "http://example.org#g36.source.reheat_overrides";
const G36_REHEAT_OVERRIDES_CLASS: &str = "http://example.org#Buildings.Controls.OBC.ASHRAE.G36.TerminalUnits.Reheat.Subsequences.Overrides";

fn import_ok(src: &str) -> ModelGraph {
    let (graph, report) = import_cxf(src.as_bytes(), &ResolveOptions::default())
        .expect("G36 Reheat Overrides fixture should import");
    assert!(
        report.is_empty(),
...
#[test]
fn source_verified_reheat_overrides_preserves_topology_and_bindings() {
    let parsed: JsonValue = serde_json::from_str(G36_REHEAT_OVERRIDES).expect("G36 fixture JSON");
    let top = parsed["@graph"]
        .as_array()
        .expect("@graph array")
        .iter()
        .find(|node| node["@id"] == json!(G36_REHEAT_OVERRIDES_MODEL))
        .expect("top G36 Reheat Overrides composite node");
    assert_eq!(top["@type"], json!(G36_REHEAT_OVERRIDES_CLASS));
    assert_eq!(
    ...
    let graph = import_ok(G36_REHEAT_OVERRIDES);
    ...
    assert!(block_param(&graph, ".conInt3", "k").bit_eq(&Value::Integer(1)));
    assert!(block_param(&graph, ".conInt4", "k").bit_eq(&Value::Integer(2)));
    for (suffix, real_true, real_false) in [
        (".cloDam", 0.0, 0.0),
        (".opeDam", 1.0, 0.0),
        (".booToRea", 0.0, 1.0),
    ] {
        assert!(block_param(&graph, suffix, "realTrue").bit_eq(&Value::Real(real_true)));
        assert!(block_param(&graph, suffix, "realFalse").bit_eq(&Value::Real(real_false)));
    }
}
...
fn reheat_overrides_modelgraph_is_stable() {
    ...
    assert_eq!(
        actual, expected,
        "source-verified G36 Reheat Overrides ModelGraph diverged from golden"
    );
}
