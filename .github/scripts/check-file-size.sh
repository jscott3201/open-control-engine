#!/usr/bin/env bash
# 700-line cap per source file. Counts non-empty, non-comment lines in tracked Rust
# source files; flags any that exceed the cap. Runs from repo root. Compatible with
# macOS bash 3.x (no mapfile).
