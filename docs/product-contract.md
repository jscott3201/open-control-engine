...
| PC-002 | CURRENT | Product claimant | Release maintainers | MUST preserve oce-api as host facade, oce-store as conditional adapter port and oce-blocks catalog as transitional companion under the delegated package and surface classifications. | Nothing is published; stable-candidate is not stable SemVer. Placeholder loaders are not working loaders; implementation dependencies are not independently supported APIs. | [Packages](package-publication-policy.md#closed-package-matrix); [Surface](public-surface-contract.md#surface-ruling); [Loaders](../crates/oce-api/src/loading.rs#L59-L96) | test [test_ratified_publish_and_private_sets_are_exact](../scripts/package_policy/test_validate.py#L181-L191); [test_owner_approved_categories_cannot_drift_with_same_publish_bit](../scripts/package_policy/test_validate.py#L193-L209); [deferred_load_paths_return_typed_errors_not_panics](../crates/oce-api/src/tests/frozen_surface.rs#L198-L218) |
| PC-003 | CURRENT | Product claimant | CXF maintainers | MUST constrain executable CXF claims to the closed, bounded, already-specialized graph profile, including the documented bounded composite lowering. | Not general Modelica flattening, source recovery, every CXF construct, or promotion of every accepted syntax to stable support. Semantic and traversal bounds do not bound serialized bytes. | [Load pipeline](../crates/oce-api/src/engine.rs#L221-L257); [Subset](cxf-composite-subset.md#active-nodes); [Deferred Modelica](../crates/oce-api/src/loading.rs#L82-L96) | test [composite_nesting_accepts_the_limit_and_rejects_one_past](../crates/oce-cxf/tests/ingest_totality.rs#L414-L429); [boundary_hops_accept_the_limit_and_reject_the_attempted_next_hop](../crates/oce-cxf/tests/ingest_totality.rs#L465-L486); [deferred_load_paths_return_typed_errors_not_panics](../crates/oce-api/src/tests/frozen_surface.rs#L198-L218) |
...
| PC-017 | CURRENT | Product claimant | Facade maintainers | MUST describe current realtime assertion delivery as warning-only collection, with tick and simulation using a no-op diagnostic sink. | AssertLevel::Error exists but is not emitted by this collector; assertions are not a safety interlock. A failed write can prevent report delivery. | [Collector](../crates/oce-api/src/sim.rs#L420-L433); [No-op tick](../crates/oce-api/src/engine.rs#L274-L277) | test [step_realtime_delivers_assert_diagnostics_and_simulate_drops_them](../crates/oce-api/src/assert_tests.rs#L78-L101) |
...
| PC-028 | FUTURE | Facade delivery | Facade maintainers | MUST remove or quarantine deferred and panic-only supported surfaces with coordinated compatibility evidence. | Not implemented here; no placeholder is promoted by having a signature. | [Current ruling](public-surface-contract.md#surface-ruling) | future [M01-PR02](#facade-quarantine) |
| PC-029 | FUTURE | Facade delivery | Facade maintainers | MUST expose versioned facade catalog, diagnostics, IO, values, assertions and execution-profile contracts with compatibility tests. | No descriptor or new severity behavior exists by virtue of this document. | [Current ruling](public-surface-contract.md#surface-ruling) | future [M01-PR03](#facade-schemas) |
...
| PC-031 | FUTURE | Frame delivery | Execution maintainers | MUST define the complete generation-atomic typed input/output frame contract. | Not the present sparse API and not an actuator transaction. | [Current staging](../crates/oce-api/src/sim.rs#L621-L646) | future [M02-PR01](#complete-frame-contract) |
...
| PC-034 | FUTURE | Convenience delivery | Execution maintainers | MUST route simulation and realtime execution through shared transition semantics or explicitly document the weaker convenience profile. | No silent retrofit of whole-horizon rollback or store-write rollback. | [Current simulation](../crates/oce-api/src/sim.rs#L451-L492) | future [M02-PR04](#shared-convenience-core) |
...
links into ignored specifications or declarations that the work has shipped. Order is facade
quarantine, facade schemas, admission/replacement, frames, then identity/state/replay qualification.
Execution of any later work still requires its own accepted prerequisite and owner authorization.

### Facade quarantine

M01-PR02: Remove or quarantine deferred loaders and panic-only supported exposure. Retain an
explicit compatibility/migration account and tests that distinguish unavailable from working paths.

### Facade schemas

M01-PR03: Version the facade-facing catalog, diagnostic, IO, value, assertion and execution-profile
schemas, define severity behavior, and retain compatibility evidence without duplicating signatures.
...
Only the pending new contract and its two checker files can be non-ignored untracked targets in a
pre-commit checkout. Existing evidence targets remain tracked. A clean clone after commit supplies
...
- Revision 1, 2026-09-05: initial bounded executable-CXF/HostTick product contract and traceability
  map, grounded at the SHA above. No runtime, signatures, catalog identities, state bytes or package
