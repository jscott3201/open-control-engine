...
- **Composite boundary lowering is iterative and resource-bounded** (#290, #298). A path may enter at most 64
  ...
  remain visible to single-assignment validation. An unreachable, active non-root boundary source
  now rejects before elision when its edge cannot be oriented, rather than disappearing with the
  boundary node. Reachable boundary chains retain their existing traversal and error precedence.
