...
- **Activity does not suppress classified parameter members** (#303). A false-guarded sibling can
  mark an active derivation-shaped leaf's parameter member inactive, but among classified
  `S231:hasInstance` members only derived ports are activity-filtered: a node-bearing parameter
  still grounds the leaf. The new accepted fixture pins `CDL.Reals.Sources.Constant.k` to the exact
  bits for `7.0`; applying the forbidden parameter filter removes that golden row and makes the
  public API reject the missing required parameter.
